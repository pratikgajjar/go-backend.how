...
- **Total round-trip.** Measured on Apple M3 Max with the
  benchmark in `.autoresearch/factlib/bench-emit/`[^12], 1 KB
...
[^12]: Reproducible from `.autoresearch/factlib/bench-emit/run.sh` in this repo (Postgres 17.6, `wal_level = logical`, `synchronous_commit = on`).
