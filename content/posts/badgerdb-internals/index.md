...
**Per-block, not per-table, bloom filters on the vlog.** Even a coarse bloom on every 4 MiB vlog chunk would let `Get` short-circuit the vlog read on stale-version misses. The vlog is append-only and immutable per-fid, so the bloom can be built when the file rolls and stored in a sidecar. Storage cost via [`y.BloomBitsPerKey`][bloom] at 1% FPR is `-ln(0.01)/ln(2)² ≈ 9.6 bits/key`; for 1 KB avg entries that's `4 MiB / 1 KB = 4096 entries × 9.6 / 8 = 4915 B ≈ 5 KB` per chunk — 0.12% of vlog footprint. (cost: 1–2 weeks)
