...
**Per-call latency (txn-per-Set, the same harness with `-b 1`):** on 1M × 128 B at low concurrency, p50 = 4 µs, p99 = 16 µs, p99.9 = 26 µs (captured via `time.Since` around each `db.Update`). At 1 KB the same workload moves to p50 = 4 µs, p99 = 25 µs, p99.9 = 646 µs — the long tail is the L0 stall surfacing as a 100×-bigger p99.9 even though the median single-Set is unchanged. With `-b 100` (100 entries per txn) the per-call latency reflects the batch and grows to p50 = 96 µs / p99 = 299 µs / p99.9 = 7.3 ms; throughput climbs to 832 K ops/s on the same hardware. The "single-Set" latency reading is from the unbatched txn path; throughput numbers in the table above are from `WriteBatch`.
