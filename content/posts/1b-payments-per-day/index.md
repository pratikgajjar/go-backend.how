...
This is the fundamental tax of per-row ACID commits. TigerBeetle is also ACID compliant — it amortizes the same durability cost across 8,190-transfer batches (one WAL prepare per batch). PostgreSQL, in the single-row INSERT pattern, pays **0.42 fsyncs per row** with no batching.
...
- **Zero `fsync()` syscalls, fully ACID.** 100,000 transfers committed in 4,241 total syscalls (4,225 `io_uring_enter`, 14 `sendto`, 2 `write`). Durability is in the write itself (O_DSYNC); the throughput delta vs PostgreSQL is batching — one durable WAL prepare per 8,190 transfers.
...
PostgreSQL:   one fdatasync per row commit (no batching)         →  13K RPS   (fsync-bound)
                  ...
                  same ACID guarantees -- different batching model, not a knob
```

Both systems provide full ACID durability. TigerBeetle's win is the batched interface: clients hand it up to 8,190 transfers and one durable WAL prepare commits the whole batch. PostgreSQL *has* to `fdatasync` on every commit when `synchronous_commit = on` — turning it off gives you faster COMMITs at the price of losing the last few hundred ms of committed data on crash.
