...
L2 over 128 floats (~10 cycles per dim × 128 dims × 0.33 ns =
420 ns per row, but most of the time is buffer-walking the heap, not
the math). At 10 million rows, exact-scan would be ~1.1 s; at
100 million it's 11 s, which is when "approximate is fine" stops
being a debate.
...
dimension), the in-memory graph is about
`1M × (1536 × 4 + 33 × 6) × 1.2 ≈ 7.6 GB` once you include neighbour
arrays and overhead. Below that, you're in slow mode. Production
deployments with 100M-row tables can't just `CREATE INDEX`; they have
to plan for it.
