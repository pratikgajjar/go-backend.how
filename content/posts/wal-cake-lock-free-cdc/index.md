...
| Page index for ts, lsn | enabled | Without page index, pushdown prunes whole row groups only (each batch is one row group here — a `WHERE timestamp BETWEEN …` query then reads the entire file or skips it). Page index lets the reader skip individual data pages within the row group, scanning only time-overlapping pages. |
