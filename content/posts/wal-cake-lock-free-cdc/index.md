...
| Page index for ts, lsn | enabled | Without page index, predicate pushdown can only prune whole row groups (each batch is one row group here, so without page index a `WHERE timestamp BETWEEN ...` query reads the entire file or skips it entirely). Page index lets the reader skip individual data pages within the row group, paying CPU to scan only the time-overlapping pages. |
