...
| Setting | Choice | Why |
|---|---|---|
| Codec | ZSTD level 3 | Sweet spot for CDC payloads. Per the [zstd README benchmarks](https://github.com/facebook/zstd#benchmarks), ZSTD-3 is several times faster to compress than gzip at a comparable ratio, and tighter than snappy by a meaningful margin (the exact margin depends on the column mix). Higher ZSTD levels (10+) trade order-of-magnitude more CPU for single-digit-percent size gains. |
...
| Page index for ts, lsn | enabled | Without page index, predicate pushdown skips at row-group granularity only (typically 128 MB chunks). Page index brings that down to data-page granularity (Parquet default `~1 MB` per page; smaller is configurable). The win is order-of-magnitude fewer bytes scanned for a narrow time-range predicate. |
