...
Distroless ships a **67-byte** layer. Sixty-seven. That's a single tar entry — let's look. Pulling the layer down and `tar tvf`'ing it shows it's a single 4-byte symlink: `/etc/os-release → ../usr/lib/os-release`. Each tiny structural file (`/tmp` dir, `/var/run` symlink, `/home` empty dir) is its own layer because Google's `bazel`-driven build emits one rule per file. Distroless is a *thirteen*-step tarball walk, even though the total content is under a megabyte.
