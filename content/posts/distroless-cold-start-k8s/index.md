...
Distroless ships a **67-byte** layer. Sixty-seven. Pulled and `tar tvf`'d, that layer contains exactly one entry: an empty `./` root directory tarball stub — the kind of placeholder a `bazel`-driven build emits when a target produces zero files but still needs a layer marker. The next four layers, in size order, are similarly tiny: 80 B is `/tmp/`, 123 B is `/home/` + `/home/nonroot/`, 162 B is `/etc/group` (64 bytes of content), 188 B is `/etc/passwd` (149 bytes). Each structural directory or one-line config is its own layer because Google's `bazel` build emits one rule per file. Distroless is a *thirteen*-step tarball walk, even though the total content is under a megabyte.

The largest distroless layer (288 KB) is `usr/share/zoneinfo` — the Olson timezone DB. The 254 KB layer is `etc/ssl/certs/ca-certificates.crt`, the Mozilla CA bundle (Debian-packaged). The 137 KB layer is `etc/ssl` directory metadata + per-CA hashed symlinks. Together those three layers are 84 % of distroless-static, and they're the exact things a Go HTTPS client needs.
