...
PG_ALLOWLIST = {
    ...
    "seq_scan", "seq_tup_read", "n_tup_ins", "n_tup_upd", "n_tup_del",
...
def dupe_word_defects(body: str) -> int:
    """Catch accidental "of of", "the the" duplications. Skips code blocks."""
    prose = re.sub(r"```[^\n]*\n.*?```", "", body, flags=re.DOTALL)
    prose = re.sub(r"`[^`]+`", "", prose)  # strip inline code
    n = 0
    for m in DUPE_WORD_RE.finditer(prose):
        w = m.group(1).lower()
        if w in {"that"}:  # legitimate "noted that that was..."
            continue
        n += 1
        s = max(0, m.start() - 30)
        e = min(len(prose), m.end() + 30)
        print(f"DEBUG dupe_word: {w!r} in {prose[s:e]!r}", file=sys.stderr)
    return n
